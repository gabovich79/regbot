    ...
    for attempt in range(2):
        ...
        checked = await gateway.json('verify', {
            'task':'Independently verify claims against literal evidence. Check numbers, conditions, exceptions, applicability dates, '
                   'conflicting versions and missing question aspects. Return checks [{index,supported,scope_preserved,temporal_conflict,qualifications_preserved,reason}], '
                   ...
                   'Compare units to ORIGINAL evidence: complete=false if extraction lost any limiting condition, exception or scope '
                   'or if applicability_dates mistake a procedural deadline, publication date or inferred year for legal commencement/expiry. '
                   'A claim cannot broaden scope, change AND/OR conditions, or claim a different period. '
                   ...
                   'temporal_conflict=false when the display_text explicitly says validity is unknown and no current entitlement or year is asserted. '
                   'Unknown validity is not a contradiction; application code keeps the answer partial. Explain any temporal_conflict in reason. '
